...
function Parties({token,toast}){
  ...
  "Sundry Creditors"].map(t=><option key={t}>{t}</option>)}</select></div>
...
function Products({token,toast}){
  ...
  try {
    if (editing) {
      await api(`/products/${editing.id}`, "PUT", payload, token);  // ✅ use payload
      toast("Updated", "success");
    } else {
      await api("/products", "POST", payload, token);               // ✅ use payload
      toast("Added", "success");
    }
    setShowModal(false);
    load();
