...
function Parties({token,toast,companyId}){
  ...
  const save=async()=>{if(!form.name)return toast("Party name required","error");setSaving(true);try{if(editing){await api(`/parties/${editing.id}`,"PUT",form,token);toast("Updated","success");}else{await api("/parties","POST",{...form,company_id:companyId||null},token);toast("Party added","success");}setShowModal(false);load();}catch(e){toast(e.message,"error");}setSaving(false);};
...
function InvoiceForm({token,toast,type,onClose,onSave,companyId}){
  ...
  const save=async()=>{
    ...
    setSaving(true);
    try{await api("/invoices","POST",{...form,company_id:companyId||null,items:items.map(item=>{const c=calcItem(item);return{...item,...c};})},token);toast(`${type==="SALES"?"Invoice":"Bill"} created!`,"success");onSave();}
    catch(e){toast(e.message,"error");}
    setSaving(false);
  };
  return(
      ...
      <div style={S.twoCol}>
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            ...
            <div style={S.fg}><label style={S.label}>HSN/SAC <span style={{fontSize:10,color:"#8B949E"}}>(auto-fills name, GST, unit)</span></label><HSNInput value={form.hsn_sac||""} onChange={v=>setForm(p=>({...p,hsn_sac:v}))} token={token} onSelect={item=>{setForm(p=>({...p,hsn_sac:item.code,name:p.name||item.description||p.name,gst_rate:String(item.gst_rate||p.gst_rate||18),unit:item.uom||p.unit||"PCS",category:p.category||(item.chapter?"Chapter "+item.chapter:"")}));toast&&toast("Auto-filled from HSN: "+item.description,"success");}} placeholder="Type HSN code to auto-fill..."/></div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
  ...
  return(<div>
    ...
    {viewMode==="upload"&&(<div>
      ...
      {step===2&&preview&&(<div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>{[{l:"Transactions",v:preview.total_txns,c:"#58a6ff"},{l:"Total Debit",v:fmtM(preview.total_debit),c:"#f85149"},{l:"Total Credit",v:fmtM(preview.total_credit),c:"#3fb950"},{l:"Net",v:fmtM(preview.total_credit-preview.total_debit),c:preview.total_credit>=preview.total_debit?"#3fb950":"#f85149"}].map(k=>(<div key={k.l} style={{...S.kpi,textAlign:"center"}}><div style={S.kpiLabel}>{k.l}</div><div style={{fontSize:k.l==="Transactions"?22:13,fontWeight:700,color:k.c}}>{k.v}</div></div>))}</div><div style={{...S.card,background:"#0d2818",border:"1px solid #238636",marginBottom:12}}><div style={{fontSize:12,color:"#3fb950"}}>✅ AI auto-categorized {preview.total_txns} transactions</div></div><div style={S.card}><div style={{fontSize:13,fontWeight:600,color:"#E6EDF3",marginBottom:10}}>Preview (first 50)</div><table style={S.tbl}><thead><tr>{["Date","Narration (from Bank PDF)","Debit (Withdrawal)","Credit (Deposit)","Category","Type"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(preview.transactions||[]).slice(0,50).map((t,i)=>(<tr key={i}><td style={S.td}>{t.txn_date}</td><td style={{...S.td,maxWidth:250}}>
              ...
              <span style={{fontSize:10,color:"#8B949E"}}>{t.category}</span>
            </td><td style={{...S.td,color:"#f85149"}}>{t.debit>0?fmtM(t.debit):"—"}</td><td style={{...S.td,color:"#3fb950"}}>{t.credit>0?fmtM(t.credit):"—"}</td><td style={S.td}>{badge(t.category||"Uncategorized",t.type==="INCOME"?"green":t.type==="EXPENSE"?"red":t.type==="TAX"?"amber":"gray")}</td>
                <td style={S.tdL}>{badge(t.type||"UNKNOWN","gray")}</td></tr>))}</tbody></table></div><div style={{display:"flex",gap:10}}><button onClick={reset} style={{...S.btnGhost,flex:1}}>Back</button><button onClick={importDB} disabled={importing} style={{...S.btnG,flex:2,opacity:importing?0.5:1}}>{importing?"Importing...":"Import All"}</button></div></div>)}
