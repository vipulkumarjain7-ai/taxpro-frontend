    ...
    <div style={{ minHeight:"100vh", background:"#0D1117", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'Inter',sans-serif" }}>
      <div style={{ width:"min(420px,90vw)" }}>
        <div style={{ textAlign:"center", marginBottom:32 }}>
          ...
          <div style={{ fontSize:13, color:"#8B949E", marginTop:6 }}>CA Practice Suite</div>
        </div>
        ...
        <div style={{ textAlign:"center", marginTop:16, fontSize:12, color:"#8B949E" }}>TaxPro GST - Secure - GST Compliant</div>
      </div>
    </div>
  );
}
...
function Dashboard({ token }) {
  ...
  if (!data) return <div style={{ color:"#f85149", padding:20 }}>Failed to load. Is backend running?</div>;
    ...
    <div>
      <div style={S.kpiGrid}>
        {[
          ...
          { label:"Compliant",      val:clients.compliant,      sub:`${clients.total?Math.round(clients.compliant/clients.total*100):0}% on time`, color:"#3fb950" },
          { label:"Open Notices",   val:notices.open,           sub:"require action",      color:"#f85149" },
          { label:"Due in 30 Days", val:notices.due_in_30_days, sub:"expiring soon",       color:"#e3b341" },
        ].map(k => (
      ...
      <div style={S.twoCol}>
        <div style={S.card}>
          ...
          {recent_clients.length === 0
            ? <div style={{ color:"#8B949E", fontSize:12 }}>No clients yet. Add your first client.</div>
...
function Clients({ token, toast }) {
  ...
  const del = async (id) => {
    ...
    try { await api(`/clients/${id}`,"DELETE",null,token); toast("Deleted","success"); load(); }
    catch(e) { toast(e.message,"error"); }
  };
                ...
                <tbody>
                  {clients.map(c=>(
                    <tr key={c.id}>
                      ...
                      <td style={S.tdLast}><div style={{ display:"flex",gap:6 }}><button onClick={()=>openEdit(c)} style={S.btnGhost}>Edit</button><button onClick={()=>del(c.id)} style={S.btnDanger}>Del</button></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {showModal && (
        <Modal title={editing?"Edit Client":"Add Client"} onClose={()=>setShowModal(false)}>
          {[{l:"Client Name *",k:"name",ph:"Sharma Textiles Pvt Ltd"},{l:"GSTIN *",k:"gstin",ph:"09AABCS1429B1Z7"},{l:"Turnover",k:"turnover",ph:"2.4 Cr"}].map(f=>(
            <div key={f.k} style={S.formGroup}><label style={S.label}>{f.l}</label><input style={S.input} placeholder={f.ph} value={form[f.k]} onChange={e=>setForm(p=>({...p,[f.k]:e.target.value}))} /></div>
          ))}
    ...
    <div>
      ...
      {showModal && (
        <Modal title="Add Notice" onClose={()=>setShowModal(false)}>
          ...
          {[{l:"Ref No. *",k:"ref_no",ph:"ZD071125006543C"},{l:"Notice Type *",k:"type",ph:"GSTR-1 vs 3B Mismatch"},{l:"Amount *",k:"amount",ph:"124500",t:"number"}].map(f=>(
          ...
          <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
            <button onClick={()=>setShowModal(false)} style={S.btnGhost}>Cancel</button>
            <button onClick={save} disabled={saving} style={{ ...S.btn, opacity:saving?0.6:1 }}>{saving?"Saving...":"Add"}</button>
          </div>
        </Modal>
      )}
    </div>
...
function Returns({ token, toast }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast("Saved","success"); setShowModal(false); load();
    } catch(e) { toast(e.message,"error"); }
    setSaving(false);
  };
  ...
  return (
    <div>
      ...
      {loading ? <Spinner /> : (
        <div style={S.card}>
          {returns.length===0
            ? <div style={{ textAlign:"center", padding:40, color:"#8B949E" }}>No records for {period}.</div>
            : <table style={S.tbl}>
      ...
      {showModal && (
        <Modal title="Add Return Record" onClose={()=>setShowModal(false)}>
          ...
          {[["gstr1_status","GSTR-1"],["gstr3b_status","GSTR-3B"],["gstr9_status","GSTR-9"]].map(([key,lbl])=>(
            <div key={key} style={S.formGroup}><label style={S.label}>{lbl} Status</label>
...
function Reconciliation({ token, toast }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast("Added","success"); setShowModal(false); setForm({ vendor_name:"", vendor_gstin:"", invoice_count:"", gstr2a_amount:"", gstr2b_amount:"", books_amount:"", remarks:"" }); load();
    } catch(e) { toast(e.message,"error"); }
    setSaving(false);
  };
  ...
  return (
    <div>
      ...
      {loading ? <Spinner /> : (
        <div style={S.card}>
          {!data||data.rows.length===0
            ? <div style={{ textAlign:"center", padding:40, color:"#8B949E" }}>No entries yet.</div>
            : <table style={S.tbl}>
      ...
      {showModal && (
        <Modal title="Add Entry" onClose={()=>setShowModal(false)}>
          {[{l:"Vendor Name *",k:"vendor_name",ph:"ABC Suppliers Pvt Ltd"},{l:"Vendor GSTIN *",k:"vendor_gstin",ph:"07AABCA1234B1Z5"},{l:"Invoice Count",k:"invoice_count",ph:"12",t:"number"},{l:"GSTR-2A Amount",k:"gstr2a_amount",ph:"145000",t:"number"},{l:"GSTR-2B Amount",k:"gstr2b_amount",ph:"143000",t:"number"},{l:"Books Amount *",k:"books_amount",ph:"147000",t:"number"}].map(f=>(
            <div key={f.k} style={S.formGroup}><label style={S.label}>{f.l}</label><input style={S.input} type={f.t||"text"} placeholder={f.ph} value={form[f.k]} onChange={e=>setForm(p=>({...p,[f.k]:e.target.value}))} /></div>
          ))}
          <div style={{ display:"flex", gap:8, justifyContent:"flex-end" }}>
            <button onClick={()=>setShowModal(false)} style={S.btnGhost}>Cancel</button>
      ...
      <div style={S.aiMsgs}>
        {msgs.map((m,i)=>(
          <div key={i} style={{ display:"flex", justifyContent:m.role==="user"?"flex-end":"flex-start" }}>
            {m.role==="assistant"&&<div style={{ width:28,height:28,borderRadius:"50%",background:"#1F6FEB",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,marginRight:8,flexShrink:0,marginTop:2,color:"#fff",fontWeight:700 }}>AI</div>}
            <div style={m.role==="user"?S.bubbleUser:S.bubbleBot}>{m.content}</div>
          </div>
        ))}
        {loading&&<div style={{ display:"flex",gap:8 }}><div style={{ width:28,height:28,borderRadius:"50%",background:"#1F6FEB",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,color:"#fff",fontWeight:700 }}>AI</div><div style={{ ...S.bubbleBot,color:"#8B949E" }}>Thinking...</div></div>}
        <div ref={endRef} />
      </div>
...
const NAV = [
  ...
  {key:"clients",icon:"C",label:"Clients"},
  {key:"reconciliation",icon:"R",label:"Reconciliation"},
  {key:"notices",icon:"N",label:"Notice Manager"},
  {key:"returns",icon:"F",label:"Return Tracker"},
  {key:"ai",icon:"A",label:"AI Assistant"},
];
        ...
        <nav style={{ flex:1, padding:"8px 0" }}>
          {NAV.map(n=>(
            <button key={n.key} onClick={()=>setView(n.key)} style={{ display:"flex", alignItems:"center", gap:10, width:"100%", padding:"9px 16px", border:"none", background:view===n.key?"#1F6FEB18":"transparent", borderLeft:view===n.key?"2px solid #1F6FEB":"2px solid transparent", color:view===n.key?"#58a6ff":"#8B949E", cursor:"pointer", fontFamily:"inherit", fontSize:13, fontWeight:view===n.key?600:400, textAlign:"left" }}>
              <span style={{ fontSize:12, width:20, height:20, borderRadius:4, background:view===n.key?"#1F6FEB":"#21262D", display:"flex", alignItems:"center", justifyContent:"center", color:view===n.key?"#fff":"#8B949E", fontWeight:700, flexShrink:0 }}>{n.icon}</span>
              <span>{n.label}</span>
            </button>
          ))}
        </nav>
