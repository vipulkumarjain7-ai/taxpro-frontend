...
const SBadge=({s})=>{const m={compliant:["Compliant","green"],pending:["Pending","amber"],notice:["Notice","red"],overdue:["Overdue","red"],paid:["Paid","green"],unpaid:["Unpaid","red"],partial:["Partial","amber"],filed:["Filed","green"],"not-filed":["Not Filed","red"]};const[l,c]=m[s]||[s,"gray"];return badge(l,c);};
...
const Modal=({title,onClose,children,wide})=>(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.8)",zIndex:100,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:30,overflowY:"auto"}}><div style={{background:"#161B22",border:"1px solid #30363D",borderRadius:12,padding:24,width:wide?"min(880px,96vw)":"min(560px,92vw)",maxHeight:"90vh",overflowY:"auto",margin:"0 auto 30px"}}><div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:18}}><span style={{fontSize:15,fontWeight:600,color:"#E6EDF3"}}>{title}</span><button onClick={onClose} style={{background:"none",border:"none",color:"#8B949E",cursor:"pointer",fontSize:22}}>✕</button></div>{children}</div></div>);

function AuthScreen({onAuth}){
  const[tab,setTab]=useState("login");const[form,setForm]=useState({name:"",email:"",password:"",firm_name:"",frn:""});const[loading,setLoading]=useState(false);const[error,setError]=useState("");
  const set=k=>e=>setForm(f=>({...f,[k]:e.target.value}));
  const submit=async()=>{setError("");setLoading(true);try{const endpoint=tab==="login"?"/auth/login":"/auth/register";const body=tab==="login"?{email:form.email,password:form.password}:{name:form.name,email:form.email,password:form.password,firm_name:form.firm_name,frn:form.frn};const data=await api(endpoint,"POST",body);localStorage.setItem("taxpro_token",data.token);localStorage.setItem("taxpro_user",JSON.stringify(data.user));onAuth(data.user,data.token);}catch(e){setError(e.message);}setLoading(false);};
  return(<div style={{minHeight:"100vh",background:"#0D1117",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter',sans-serif"}}><div style={{width:"min(420px,92vw)"}}><div style={{textAlign:"center",marginBottom:28}}><div style={{fontSize:32,marginBottom:6}}>🛡️</div><div style={{fontSize:26,fontWeight:800,color:"#E6EDF3"}}>TaxPro GST</div><div style={{fontSize:13,color:"#8B949E",marginTop:4}}>Complete Accounting + GST Software</div></div><div style={{background:"#161B22",border:"1px solid #21262D",borderRadius:14,padding:28}}><div style={{display:"flex",gap:4,marginBottom:22,background:"#0D1117",borderRadius:10,padding:4}}>{["login","register"].map(t=>(<button key={t} onClick={()=>setTab(t)} style={{flex:1,padding:"9px",borderRadius:7,border:"none",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600,background:tab===t?"#1F6FEB":"transparent",color:tab===t?"#fff":"#8B949E"}}>{t==="login"?"Sign In":"Register"}</button>))}</div>{tab==="register"&&<><div style={S.fg}><label style={S.label}>Full Name *</label><input style={S.input} placeholder="CA Rahul Prakash" value={form.name} onChange={set("name")}/></div><div style={S.fg}><label style={S.label}>Firm Name *</label><input style={S.input} placeholder="Prakash & Associates" value={form.firm_name} onChange={set("firm_name")}/></div><div style={S.fg}><label style={S.label}>FRN (optional)</label><input style={S.input} placeholder="001234N" value={form.frn} onChange={set("frn")}/></div></>}<div style={S.fg}><label style={S.label}>Email *</label><input style={S.input} type="email" placeholder="you@firm.com" value={form.email} onChange={set("email")} onKeyDown={e=>e.key==="Enter"&&submit()}/></div><div style={S.fg}><label style={S.label}>Password *</label><input style={S.input} type="password" placeholder="min 6 characters" value={form.password} onChange={set("password")} onKeyDown={e=>e.key==="Enter"&&submit()}/></div>{error&&<div style={{background:"#2d0e0e",border:"1px solid #6e1c1c",color:"#f85149",padding:"10px 14px",borderRadius:8,fontSize:12,marginBottom:14}}>⚠️ {error}</div>}<button onClick={submit} disabled={loading} style={{...S.btn,width:"100%",padding:"12px",opacity:loading?0.6:1}}>{loading?"Please wait...":tab==="login"?"Sign In →":"Create Account"}</button></div></div></div>);
}
...
function Parties({token,toast}){
  ...
  return(<div>
    <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center",flexWrap:"wrap"}}><input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==="Enter"&&load()} placeholder="Search parties..." style={{...S.input,width:280}}/><button onClick={load} style={S.btnGhost}>Search</button><button onClick={openAdd} style={{...S.btn,marginLeft:"auto"}}>+ Add Party</button></div>
    {loading?<Spinner/>:(<div style={S.card}>{parties.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No parties yet.</div>:(<table style={S.tbl}><thead><tr>{["Name","GSTIN","Type","Phone","City","Outstanding","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{parties.map(p=>(<tr key={p.id}><td style={{...S.td,fontWeight:600,color:"#58a6ff",cursor:"pointer"}} onClick={()=>viewLedger(p.id)}>{p.name}</td><td style={S.td}><span style={S.mono}>{p.gstin||"—"}</span></td><td style={S.td}>{badge(p.type,p.type==="Customer"?"green":p.type==="Supplier"?"blue":"gray")}</td><td style={S.td}>{p.phone||"—"}</td><td style={S.td}>{p.city||"—"}</td><td style={{...S.td,color:parseFloat(p.outstanding||0)>0?"#e3b341":"#3fb950",fontWeight:600}}>{fmtM(p.outstanding||0)}</td><td style={S.tdL}><div style={{display:"flex",gap:4}}><button onClick={()=>viewLedger(p.id)} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Ledger</button><button onClick={()=>openEdit(p)} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Edit</button><button onClick={()=>del(p.id)} style={{...S.btnDanger,fontSize:11,padding:"4px 8px"}}>Del</button></div></td></tr>))}</tbody></table>)}</div>)}
    {showModal&&(<Modal title={editing?"Edit Party":"Add Party"} onClose={()=>setShowModal(false)} wide><div style={S.twoCol}><div><div style={S.fg}><label style={S.label}>Party Name *</label><input style={S.input} placeholder="Company or person name" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Type</label><select style={S.select} value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value}))}>{["Customer","Supplier","Both"].map(t=><option key={t}>{t}</option>)}</select></div><div style={S.fg}><label style={S.label}>GSTIN</label><input style={S.input} placeholder="15 char GSTIN" value={form.gstin} onChange={e=>setForm(p=>({...p,gstin:e.target.value.toUpperCase()}))}/></div><div style={S.fg}><label style={S.label}>PAN</label><input style={S.input} value={form.pan} onChange={e=>setForm(p=>({...p,pan:e.target.value.toUpperCase()}))}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>Phone</label><input style={S.input} value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Email</label><input style={S.input} value={form.email} onChange={e=>setForm(p=>({...p,email:e.target.value}))}/></div></div></div><div><div style={S.fg}><label style={S.label}>Address</label><textarea style={{...S.input,resize:"vertical",minHeight:60}} value={form.address} onChange={e=>setForm(p=>({...p,address:e.target.value}))}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>City</label><input style={S.input} value={form.city} onChange={e=>setForm(p=>({...p,city:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Pincode</label><input style={S.input} value={form.pincode} onChange={e=>setForm(p=>({...p,pincode:e.target.value}))}/></div></div><div style={S.fg}><label style={S.label}>State</label><select style={S.select} value={form.state} onChange={e=>setForm(p=>({...p,state:e.target.value}))}><option value="">Select</option>{STATES.map(s=><option key={s}>{s}</option>)}</select></div><div style={S.fg}><label style={S.label}>Credit Limit (Rs.)</label><input style={S.input} type="number" value={form.credit_limit} onChange={e=>setForm(p=>({...p,credit_limit:e.target.value}))}/></div></div></div><div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={()=>setShowModal(false)} style={S.btnGhost}>Cancel</button><button onClick={save} disabled={saving} style={{...S.btn,opacity:saving?0.6:1}}>{saving?"Saving...":"Save"}</button></div></Modal>)}
    {ledger&&(<Modal title={`Ledger — ${ledger.party?.name}`} onClose={()=>setLedger(null)} wide><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:14}}>{[{l:"Total Sales",v:fmtM(ledger.summary?.total_sales||0),c:"#3fb950"},{l:"Total Purchases",v:fmtM(ledger.summary?.total_purchases||0),c:"#58a6ff"},{l:"Outstanding",v:fmtM(ledger.summary?.outstanding||0),c:"#f85149"}].map(k=>(<div key={k.l} style={S.kpi}><div style={S.kpiLabel}>{k.l}</div><div style={{fontSize:15,fontWeight:700,color:k.c}}>{k.v}</div></div>))}</div><table style={S.tbl}><thead><tr>{["Date","Invoice No","Type","Amount","Paid","Balance"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(ledger.invoices||[]).map(inv=>(<tr key={inv.id}><td style={S.td}>{inv.invoice_date}</td><td style={{...S.td,color:"#58a6ff"}}>{inv.invoice_no}</td><td style={S.td}>{badge(inv.invoice_type,inv.invoice_type==="SALES"?"green":"blue")}</td><td style={{...S.td,fontWeight:600}}>{fmtM(inv.total_amount)}</td><td style={{...S.td,color:"#3fb950"}}>{fmtM(inv.paid_amount)}</td><td style={{...S.tdL,color:parseFloat(inv.balance_due||0)>0?"#f85149":"#3fb950",fontWeight:600}}>{fmtM(inv.balance_due)}</td></tr>))}{!ledger.invoices?.length&&<tr><td colSpan={6} style={{...S.td,textAlign:"center",color:"#8B949E",padding:20}}>No transactions</td></tr>}</tbody></table></Modal>)}
  </div>);
}
...
function InvoiceForm({token,toast,type,onClose,onSave}){
  ...
  const calcItem=item=>{const qty=parseFloat(item.qty)||0,rate=parseFloat(item.rate)||0,disc=parseFloat(item.discount_pct)||0,gstRate=parseFloat(item.gst_rate)||0;const gross=qty*rate,ta=gross-gross*disc/100;const igst=form.is_igst?ta*gstRate/100:0,cgst=!form.is_igst?ta*(gstRate/2)/100:0,sgst=!form.is_igst?ta*(gstRate/2)/100:0;return{taxable:ta,igst,cgst,sgst,total:ta+igst+cgst+sgst};};
  ...
  const save=async()=>{if(!form.party_name)return toast("Party name required","error");if(items.some(i=>!i.name))return toast("All items need a name","error");setSaving(true);try{const payload={...form,items:items.map(item=>{const c=calcItem(item);return{...item,...c};})};await api("/invoices","POST",payload,token);toast(`${type==="SALES"?"Invoice":"Bill"} created!`,"success");onSave();}catch(e){toast(e.message,"error");}setSaving(false);};
  return(<Modal title={`New ${type==="SALES"?"Sales Invoice":"Purchase Bill"}`} onClose={onClose} wide>
    <div style={S.twoCol}>
      <div><div style={S.fg}><label style={S.label}>Select Party</label><select style={S.select} onChange={e=>selectParty(e.target.value)}><option value="">-- Select from list --</option>{parties.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></div><div style={S.fg}><label style={S.label}>Party Name *</label><input style={S.input} placeholder="Customer/Supplier name" value={form.party_name} onChange={e=>setForm(f=>({...f,party_name:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>GSTIN</label><input style={S.input} value={form.party_gstin} onChange={e=>setForm(f=>({...f,party_gstin:e.target.value.toUpperCase()}))}/></div><div style={S.fg}><label style={S.label}>Address</label><textarea style={{...S.input,resize:"vertical",minHeight:50}} value={form.party_address} onChange={e=>setForm(f=>({...f,party_address:e.target.value}))}/></div></div>
      <div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>Invoice Date *</label><input style={S.input} type="date" value={form.invoice_date} onChange={e=>setForm(f=>({...f,invoice_date:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Due Date</label><input style={S.input} type="date" value={form.due_date} onChange={e=>setForm(f=>({...f,due_date:e.target.value}))}/></div></div><div style={S.fg}><label style={S.label}>Place of Supply</label><input style={S.input} value={form.place_of_supply} onChange={e=>setForm(f=>({...f,place_of_supply:e.target.value}))}/></div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><input type="checkbox" id="isIgst" checked={form.is_igst} onChange={e=>setForm(f=>({...f,is_igst:e.target.checked}))}/><label htmlFor="isIgst" style={{...S.label,marginBottom:0,cursor:"pointer"}}>Inter-State (IGST)</label></div><div style={S.fg}><label style={S.label}>Notes</label><textarea style={{...S.input,resize:"vertical",minHeight:50}} value={form.notes} onChange={e=>setForm(f=>({...f,notes:e.target.value}))}/></div></div>
    </div>
    <div style={{fontSize:13,fontWeight:600,color:"#E6EDF3",marginBottom:8}}>Items</div>
    <div style={{overflowX:"auto"}}><table style={{...S.tbl,minWidth:700}}><thead><tr>{["Product","HSN","Qty","Unit","Rate","Disc%","GST%","Amount",""].map(h=><th key={h} style={{...S.th,fontSize:10}}>{h}</th>)}</tr></thead><tbody>{items.map((item,i)=>{const c=calcItem(item);return(<tr key={i}><td style={S.td}><select style={{...S.select,fontSize:11,marginBottom:4}} onChange={e=>selectProduct(i,e.target.value)}><option value="">-- Select --</option>{products.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select><input style={{...S.input,fontSize:11}} placeholder="Item name *" value={item.name} onChange={e=>setItem(i,"name",e.target.value)}/></td><td style={S.td}><input style={{...S.input,width:70,fontSize:11}} value={item.hsn_sac} onChange={e=>setItem(i,"hsn_sac",e.target.value)}/></td><td style={S.td}><input style={{...S.input,width:60,fontSize:11}} type="number" value={item.qty} onChange={e=>setItem(i,"qty",e.target.value)}/></td><td style={S.td}><select style={{...S.select,fontSize:11,width:70}} value={item.unit} onChange={e=>setItem(i,"unit",e.target.value)}>{["PCS","KG","LTR","MTR","BOX","NOS"].map(u=><option key={u}>{u}</option>)}</select></td><td style={S.td}><input style={{...S.input,width:80,fontSize:11}} type="number" value={item.rate} onChange={e=>setItem(i,"rate",e.target.value)}/></td><td style={S.td}><input style={{...S.input,width:50,fontSize:11}} type="number" value={item.discount_pct} onChange={e=>setItem(i,"discount_pct",e.target.value)}/></td><td style={S.td}><select style={{...S.select,fontSize:11,width:70}} value={item.gst_rate} onChange={e=>setItem(i,"gst_rate",e.target.value)}>{["0","5","12","18","28"].map(r=><option key={r} value={r}>{r}%</option>)}</select></td><td style={{...S.td,fontWeight:600,color:"#3fb950"}}>{fmtM(c.total)}</td><td style={S.tdL}><button onClick={()=>removeItem(i)} style={{...S.btnDanger,padding:"3px 8px",fontSize:11}}>✕</button></td></tr>);})}</tbody></table></div>
    <button onClick={addItem} style={{...S.btnGhost,fontSize:12,marginTop:8,marginBottom:16}}>+ Add Item</button>
    <div style={{display:"flex",justifyContent:"flex-end"}}><div style={{background:"#0D1117",borderRadius:8,padding:14,width:280}}>{[["Taxable",totals.taxable],...(form.is_igst?[["IGST",totals.igst]]:[["CGST",totals.cgst],["SGST",totals.sgst]]),["Total Tax",totals.igst+totals.cgst+totals.sgst]].map(([l,v])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #21262D"}}><span style={{color:"#8B949E",fontSize:12}}>{l}</span><span style={{color:"#C9D1D9",fontSize:12}}>{fmtM(v)}</span></div>))}<div style={{display:"flex",justifyContent:"space-between",padding:"8px 0"}}><span style={{color:"#E6EDF3",fontWeight:700}}>TOTAL</span><span style={{color:"#3fb950",fontWeight:700,fontSize:16}}>{fmtM(totals.total)}</span></div></div></div>
    <div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:16}}><button onClick={onClose} style={S.btnGhost}>Cancel</button><button onClick={save} disabled={saving} style={{...S.btnG,opacity:saving?0.6:1}}>{saving?"Creating...":"Create "+(type==="SALES"?"Invoice":"Bill")}</button></div>
  </Modal>);
}
...
function InvoiceList({token,toast,type}){
  ...
  const printInv=inv=>{const w=window.open("","_blank");w.document.write(`<!DOCTYPE html><html><head><title>${inv.invoice_no}</title><style>body{font-family:Arial;margin:20px;font-size:12px;}h1{color:#1F6FEB;}table{width:100%;border-collapse:collapse;margin:10px 0;}th{background:#1F6FEB;color:white;padding:7px;}td{padding:6px;border-bottom:1px solid #eee;}.total{font-weight:bold;font-size:14px;}.right{text-align:right;}</style></head><body><h1>🛡️ TaxPro GST — ${type==="SALES"?"TAX INVOICE":"PURCHASE BILL"}</h1><p><strong>Invoice No:</strong> ${inv.invoice_no} &nbsp; <strong>Date:</strong> ${inv.invoice_date}</p><p><strong>Party:</strong> ${inv.party_name} &nbsp; ${inv.party_gstin?`<strong>GSTIN:</strong> ${inv.party_gstin}`:""}}</p><p>${inv.party_address||""}</p><table><thead><tr><th>#</th><th>Item</th><th>HSN</th><th>Qty</th><th>Rate</th><th>Taxable</th><th>GST%</th><th>Tax</th><th>Total</th></tr></thead><tbody>${(inv.items||[]).map((it,i)=>`<tr><td>${i+1}</td><td>${it.name}</td><td>${it.hsn_sac||""}</td><td>${it.qty} ${it.unit}</td><td>${fmtM(it.rate)}</td><td>${fmtM(it.taxable_value)}</td><td>${it.gst_rate}%</td><td>${fmtM((it.igst_amount||0)+(it.cgst_amount||0)+(it.sgst_amount||0))}</td><td><b>${fmtM(it.total_amount)}</b></td></tr>`).join("")}</tbody></table><div class="right"><p>Taxable: ${fmtM(inv.taxable_amount)}</p>${inv.is_igst?`<p>IGST: ${fmtM(inv.igst_amount)}</p>`:`<p>CGST: ${fmtM(inv.cgst_amount)} &nbsp; SGST: ${fmtM(inv.sgst_amount)}</p>`}<p class="total">TOTAL: ${fmtM(inv.total_amount)}</p><p>Paid: ${fmtM(inv.paid_amount)} &nbsp; Balance: ${fmtM(inv.balance_due)}</p></div></body></html>`);w.document.close();w.print();};
  const label=type==="SALES"?"Invoice":"Bill";
  return(<div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:14}}>{[{l:"Total "+label+"s",v:invoices.length,c:"#58a6ff"},{l:"Total Amount",v:fmtM(invoices.reduce((a,i)=>a+parseFloat(i.total_amount||0),0)),c:"#3fb950"},{l:"Outstanding",v:fmtM(invoices.reduce((a,i)=>a+parseFloat(i.balance_due||0),0)),c:"#e3b341"}].map(k=>(<div key={k.l} style={S.kpi}><div style={S.kpiLabel}>{k.l}</div><div style={{fontSize:k.l.includes("Total "+label)? 22:14,fontWeight:700,color:k.c}}>{k.v}</div></div>))}</div>
    <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center",flexWrap:"wrap"}}><input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==="Enter"&&load()} placeholder={`Search ${label.toLowerCase()}s...`} style={{...S.input,width:280}}/><button onClick={load} style={S.btnGhost}>Search</button><button onClick={()=>setShowForm(true)} style={{...S.btn,marginLeft:"auto"}}>+ New {label}</button></div>
    {loading?<Spinner/>:(<div style={S.card}>{invoices.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No {label.toLowerCase()}s yet.</div>:(<table style={S.tbl}><thead><tr>{["Invoice No","Party","Date","Due","Amount","Tax","Paid","Balance","Status","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{invoices.map(inv=>(<tr key={inv.id}><td style={{...S.td,color:"#58a6ff",cursor:"pointer",fontWeight:600}} onClick={()=>viewInv(inv.id)}>{inv.invoice_no}</td><td style={S.td}><div style={{fontWeight:500,color:"#E6EDF3"}}>{inv.party_name}</div>{inv.party_gstin&&<div style={S.mono}>{inv.party_gstin}</div>}</td><td style={S.td}>{inv.invoice_date}</td><td style={{...S.td,color:inv.due_date&&inv.due_date<todayStr()&&inv.status!=="paid"?"#f85149":"#C9D1D9"}}>{inv.due_date||"—"}</td><td style={{...S.td,fontWeight:600}}>{fmtM(inv.total_amount)}</td><td style={{...S.td,color:"#e3b341"}}>{fmtM(inv.total_tax)}</td><td style={{...S.td,color:"#3fb950"}}>{fmtM(inv.paid_amount)}</td><td style={{...S.td,color:parseFloat(inv.balance_due||0)>0?"#f85149":"#3fb950",fontWeight:600}}>{fmtM(inv.balance_due)}</td><td style={S.td}><SBadge s={inv.status}/></td><td style={S.tdL}><div style={{display:"flex",gap:4}}><button onClick={()=>viewInv(inv.id)} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>View</button>{inv.status!=="paid"&&<button onClick={()=>{setPayModal(inv);setPayForm({amount:String(inv.balance_due||0),method:"CASH",reference_no:"",payment_date:todayStr()});}} style={{...S.btnG,fontSize:11,padding:"4px 8px"}}>Pay</button>}<button onClick={()=>del(inv.id)} style={{...S.btnDanger,fontSize:11,padding:"4px 8px"}}>Del</button></div></td></tr>))}</tbody></table>)}</div>)}
    {showForm&&<InvoiceForm token={token} toast={toast} type={type} onClose={()=>setShowForm(false)} onSave={()=>{setShowForm(false);load();}}/>}
    {viewing&&(<Modal title={`${type==="SALES"?"Invoice":"Bill"}: ${viewing.invoice_no}`} onClose={()=>setViewing(null)} wide><div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center"}}><button onClick={()=>printInv(viewing)} style={S.btnG}>🖨 Print/PDF</button><SBadge s={viewing.status}/>{viewing.status!=="paid"&&<div style={{marginLeft:"auto",color:"#f85149",fontWeight:600}}>Balance: {fmtM(viewing.balance_due)}</div>}</div><div style={S.twoCol}><div style={{...S.card,margin:0}}><div style={{fontSize:11,color:"#8B949E",marginBottom:4}}>Bill To</div><div style={{fontWeight:600,color:"#E6EDF3"}}>{viewing.party_name}</div>{viewing.party_gstin&&<div style={S.mono}>{viewing.party_gstin}</div>}{viewing.party_address&&<div style={{fontSize:11,color:"#8B949E",marginTop:4}}>{viewing.party_address}</div>}</div><div style={{...S.card,margin:0,fontSize:12,lineHeight:1.9}}><div><span style={{color:"#8B949E"}}>Date: </span>{viewing.invoice_date}</div>{viewing.due_date&&<div><span style={{color:"#8B949E"}}>Due: </span>{viewing.due_date}</div>}<div><span style={{color:"#8B949E"}}>Tax: </span>{viewing.is_igst?"IGST":"CGST+SGST"}</div></div></div><table style={{...S.tbl,marginTop:12}}><thead><tr>{["Item","HSN","Qty","Rate","Taxable","GST%","Tax","Total"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(viewing.items||[]).map((item,i)=>(<tr key={i}><td style={{...S.td,fontWeight:500,color:"#E6EDF3"}}>{item.name}</td><td style={S.td}>{item.hsn_sac||"—"}</td><td style={S.td}>{item.qty} {item.unit}</td><td style={S.td}>{fmtM(item.rate)}</td><td style={S.td}>{fmtM(item.taxable_value)}</td><td style={S.td}>{item.gst_rate}%</td><td style={S.td}>{fmtM((item.igst_amount||0)+(item.cgst_amount||0)+(item.sgst_amount||0))}</td><td style={{...S.tdL,fontWeight:700}}>{fmtM(item.total_amount)}</td></tr>))}</tbody></table><div style={{display:"flex",justifyContent:"flex-end",marginTop:8}}><div style={{background:"#0D1117",borderRadius:8,padding:12,width:270}}>{[["Taxable",viewing.taxable_amount],...(viewing.is_igst?[["IGST",viewing.igst_amount]]:[["CGST",viewing.cgst_amount],["SGST",viewing.sgst_amount]]),["Total Tax",viewing.total_tax],["TOTAL",viewing.total_amount,true,"#3fb950"],["Paid",viewing.paid_amount,false,"#3fb950"],["Balance Due",viewing.balance_due,true,parseFloat(viewing.balance_due||0)>0?"#f85149":"#3fb950"]].map(([l,v,bold,color])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",padding:"5px 0",borderBottom:"1px solid #21262D"}}><span style={{color:"#8B949E",fontSize:12}}>{l}</span><span style={{color:color||"#E6EDF3",fontWeight:bold?700:400,fontSize:bold?13:12}}>{fmtM(v)}</span></div>))}</div></div></Modal>)}
...
function Products({token,toast}){
  ...
  return(<div>
    <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center",flexWrap:"wrap"}}><input value={search} onChange={e=>setSearch(e.target.value)} onKeyDown={e=>e.key==="Enter"&&load()} placeholder="Search products..." style={{...S.input,width:260}}/><button onClick={load} style={S.btnGhost}>Search</button><button onClick={openAdd} style={{...S.btn,marginLeft:"auto"}}>+ Add Product</button></div>
    {loading?<Spinner/>:(<div style={S.card}>{products.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No products yet.</div>:(<table style={S.tbl}><thead><tr>{["Name","Code","HSN","GST%","Purchase","Sale Price","Stock","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{products.map(p=>(<tr key={p.id}><td style={{...S.td,fontWeight:600,color:"#E6EDF3"}}>{p.name}{p.is_service&&<span style={{fontSize:10,color:"#bf91f3",marginLeft:6}}>SVC</span>}</td><td style={S.td}><span style={S.mono}>{p.code||"—"}</span></td><td style={S.td}>{p.hsn_sac||"—"}</td><td style={{...S.td,color:"#e3b341"}}>{p.gst_rate}%</td><td style={S.td}>{fmtM(p.purchase_price)}</td><td style={{...S.td,color:"#3fb950",fontWeight:600}}>{fmtM(p.sale_price)}</td><td style={{...S.td,color:parseFloat(p.stock_qty||0)<=parseFloat(p.min_stock||0)?"#f85149":"#3fb950",fontWeight:600}}>{p.stock_qty} {p.unit}</td><td style={S.tdL}><div style={{display:"flex",gap:4}}><button onClick={()=>{setStockModal(p);setStockForm({type:"IN",qty:"",rate:"",notes:""}); }} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Stock</button><button onClick={()=>openEdit(p)} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Edit</button><button onClick={()=>del(p.id)} style={{...S.btnDanger,fontSize:11,padding:"4px 8px"}}>Del</button></div></td></tr>))}</tbody></table>)}</div>)}
    {showModal&&(<Modal title={editing?"Edit Product":"Add Product"} onClose={()=>setShowModal(false)} wide><div style={S.twoCol}><div><div style={S.fg}><label style={S.label}>Name *</label><input style={S.input} value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>Code</label><input style={S.input} value={form.code} onChange={e=>setForm(p=>({...p,code:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>HSN/SAC</label><input style={S.input} value={form.hsn_sac} onChange={e=>setForm(p=>({...p,hsn_sac:e.target.value}))}/></div></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>Unit</label><select style={S.select} value={form.unit} onChange={e=>setForm(p=>({...p,unit:e.target.value}))}>{UNITS.map(u=><option key={u}>{u}</option>)}</select></div><div style={S.fg}><label style={S.label}>GST Rate</label><select style={S.select} value={form.gst_rate} onChange={e=>setForm(p=>({...p,gst_rate:e.target.value}))}>{["0","5","12","18","28"].map(r=><option key={r} value={r}>{r}%</option>)}</select></div></div><div style={S.fg}><label style={S.label}>Category</label><input style={S.input} value={form.category} onChange={e=>setForm(p=>({...p,category:e.target.value}))}/></div><div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}><input type="checkbox" id="isSvc" checked={form.is_service} onChange={e=>setForm(p=>({...p,is_service:e.target.checked}))}/><label htmlFor="isSvc" style={{...S.label,marginBottom:0,cursor:"pointer"}}>Service (no stock)</label></div></div><div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>Purchase Price</label><input style={S.input} type="number" value={form.purchase_price} onChange={e=>setForm(p=>({...p,purchase_price:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Sale Price</label><input style={S.input} type="number" value={form.sale_price} onChange={e=>setForm(p=>({...p,sale_price:e.target.value}))}/></div></div>{!editing&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div style={S.fg}><label style={S.label}>Opening Stock</label><input style={S.input} type="number" value={form.stock_qty} onChange={e=>setForm(p=>({...p,stock_qty:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Min Stock Alert</label><input style={S.input} type="number" value={form.min_stock} onChange={e=>setForm(p=>({...p,min_stock:e.target.value}))}/></div></div>}<div style={S.fg}><label style={S.label}>Description</label><textarea style={{...S.input,resize:"vertical",minHeight:80}} value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))}/></div></div></div><div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={()=>setShowModal(false)} style={S.btnGhost}>Cancel</button><button onClick={save} disabled={saving} style={{...S.btn,opacity:saving?0.6:1}}>{saving?"Saving...":"Save"}</button></div></Modal>)}
    {stockModal&&(<Modal title={`Adjust Stock — ${stockModal.name}`} onClose={()=>setStockModal(null)}><div style={{textAlign:"center",padding:"10px 0 20px"}}><div style={{fontSize:36,fontWeight:800,color:"#E6EDF3"}}>{stockModal.stock_qty}</div><div style={{fontSize:12,color:"#8B949E"}}>Current Stock ({stockModal.unit})</div></div><div style={S.fg}><div style={{display:"flex",gap:8}}>{["IN","OUT"].map(t=>(<button key={t} onClick={()=>setStockForm(f=>({...f,type:t}))} style={{flex:1,padding:"10px",borderRadius:8,border:"1px solid",cursor:"pointer",fontFamily:"inherit",fontSize:13,fontWeight:600,borderColor:stockForm.type===t?"#238636":"#30363D",background:stockForm.type===t?"#0d2818":"transparent",color:stockForm.type===t?"#3fb950":"#8B949E"}}>{t==="IN"?"📦 Stock IN":"📤 Stock OUT"}</button>))}</div></div>{[{l:"Quantity *",k:"qty",t:"number"},{l:"Rate",k:"rate",t:"number"},{l:"Notes",k:"notes",t:"text"}].map(f=>(<div key={f.k} style={S.fg}><label style={S.label}>{f.l}</label><input style={S.input} type={f.t} value={stockForm[f.k]} onChange={e=>setStockForm(p=>({...p,[f.k]:e.target.value}))}/></div>))}<div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={()=>setStockModal(null)} style={S.btnGhost}>Cancel</button><button onClick={adjustStock} style={S.btnG}>Update Stock</button></div></Modal>)}
  </div>);
}
...
function GSTClients({token,toast}){
  ...
  return(<div><div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center"}}><input value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&load()} placeholder="Search GST clients..." style={{...S.input,width:280}}/><button onClick={load} style={S.btnGhost}>Search</button><button onClick={()=>{setEditing(null);setForm({name:"",gstin:"",state:"",type:"Trader",turnover:"",notes:"",status:"compliant"});setShowModal(true);}} style={{...S.btn,marginLeft:"auto"}}>+ Add Client</button></div>
    {loading?<Spinner/>:(<div style={S.card}>{clients.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No clients.</div>:(<table style={S.tbl}><thead><tr>{["Name","GSTIN","State","Type","Status","Notices","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{clients.map(c=>(<tr key={c.id}><td style={{...S.td,fontWeight:600,color:"#E6EDF3"}}>{c.name}</td><td style={S.td}><span style={S.mono}>{c.gstin}</span></td><td style={S.td}>{c.state}</td><td style={S.td}>{badge(c.type,"gray")}</td><td style={S.td}><SBadge s={c.status}/></td><td style={S.td}>{c.notice_count>0?<span style={{color:"#f85149",fontWeight:700}}>{c.notice_count}</span>:<span style={{color:"#3fb950"}}>0</span>}</td><td style={S.tdL}><div style={{display:"flex",gap:4}}><button onClick={()=>{setEditing(c);setForm({name:c.name,gstin:c.gstin||"",state:c.state||"",type:c.type||"Trader",turnover:c.turnover||"",notes:c.notes||"",status:c.status||"compliant"});setShowModal(true);}} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Edit</button><button onClick={()=>del(c.id)} style={{...S.btnDanger,fontSize:11,padding:"4px 8px"}}>Del</button></div></td></tr>))}</tbody></table>)}</div>)}
    {showModal&&(<Modal title={editing?"Edit GST Client":"Add GST Client"} onClose={()=>setShowModal(false)}>{[{l:"Name *",k:"name",ph:"Sharma Textiles Pvt Ltd"},{l:"GSTIN *",k:"gstin",ph:"09AABCS1429B1Z7"},{l:"Turnover",k:"turnover",ph:"2.4 Cr"}].map(f=>(<div key={f.k} style={S.fg}><label style={S.label}>{f.l}</label><input style={S.input} placeholder={f.ph} value={form[f.k]} onChange={e=>setForm(p=>({...p,[f.k]:e.target.value}))}/></div>))}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><div style={S.fg}><label style={S.label}>State</label><select style={S.select} value={form.state} onChange={e=>setForm(p=>({...p,state:e.target.value}))}><option value="">Select</option>{STATES.map(s=><option key={s}>{s}</option>)}</select></div><div style={S.fg}><label style={S.label}>Type</label><select style={S.select} value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value}))}>{["Manufacturer","Trader","Exporter","Importer","Service","Composition"].map(t=><option key={t}>{t}</option>)}</select></div></div>{editing&&<div style={S.fg}><label style={S.label}>Status</label><select style={S.select} value={form.status} onChange={e=>setForm(p=>({...p,status:e.target.value}))}>{["compliant","pending","notice","overdue"].map(s=><option key={s}>{s}</option>)}</select></div>}<div style={S.fg}><label style={S.label}>Notes</label><textarea style={{...S.input,resize:"vertical",minHeight:60}} value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))}/></div><div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={()=>setShowModal(false)} style={S.btnGhost}>Cancel</button><button onClick={save} disabled={saving} style={{...S.btn,opacity:saving?0.6:1}}>{saving?"Saving...":"Save"}</button></div></Modal>)}
  </div>);
}
...
function Notices({token,toast}){
  ...
  return(<div><div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>{["all","pending","in-progress","overdue","replied","closed"].map(t=>(<button key={t} onClick={()=>setFilter(t)} style={{padding:"5px 14px",borderRadius:20,border:"1px solid",cursor:"pointer",fontSize:12,fontFamily:"inherit",borderColor:filter===t?"#58a6ff":"#30363D",background:filter===t?"#0c1d2e":"transparent",color:filter===t?"#58a6ff":"#8B949E"}}>{t==="all"?"All":t==="in-progress"?"In Progress":t.charAt(0).toUpperCase()+t.slice(1)}</button>))}<button onClick={()=>setShowModal(true)} style={{...S.btn,marginLeft:"auto",padding:"5px 14px"}}>+ Add Notice</button></div>
    {loading?<Spinner/>:(<div style={S.card}>{notices.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No notices.</div>:(<table style={S.tbl}><thead><tr>{["Ref No","Client","Type","Due","Amount","Priority","Status",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{notices.map(n=>(<tr key={n.id}><td style={S.td}><span style={S.mono}>{n.ref_no}</span></td><td style={S.td}><div style={{fontWeight:500,color:"#E6EDF3"}}>{n.client_name}</div></td><td style={S.td}>{n.type}</td><td style={{...S.td,color:n.status==="overdue"?"#f85149":"#C9D1D9",fontWeight:n.status==="overdue"?700:400}}>{n.due_date}</td><td style={{...S.td,fontWeight:600}}>Rs.{Number(n.amount).toLocaleString("en-IN")}</td><td style={S.td}>{badge(n.priority,n.priority==="critical"||n.priority==="high"?"red":n.priority==="medium"?"blue":"gray")}</td><td style={S.td}><select value={n.status} onChange={e=>updateStatus(n.id,e.target.value)} style={{...S.select,fontSize:11,padding:"3px 6px",width:"auto"}}>{["pending","in-progress","replied","closed","overdue"].map(s=><option key={s}>{s}</option>)}</select></td><td style={S.tdL}><button onClick={()=>del(n.id)} style={S.btnDanger}>Del</button></td></tr>))}</tbody></table>)}</div>)}
    {showModal&&(<Modal title="Add Notice" onClose={()=>setShowModal(false)}><div style={S.fg}><label style={S.label}>Client *</label><select style={S.select} value={form.client_id} onChange={e=>setForm(p=>({...p,client_id:e.target.value}))}><option value="">Select</option>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div>{[{l:"Ref No *",k:"ref_no",ph:"ZD071125006543C"},{l:"Notice Type *",k:"type",ph:"GSTR-1 vs 3B Mismatch"},{l:"Amount",k:"amount",ph:"124500",t:"number"}].map(f=>(<div key={f.k} style={S.fg}><label style={S.label}>{f.l}</label><input style={S.input} type={f.t||"text"} placeholder={f.ph} value={form[f.k]} onChange={e=>setForm(p=>({...p,[f.k]:e.target.value}))}/></div>))}<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><div style={S.fg}><label style={S.label}>Issued</label><input style={S.input} type="date" value={form.issued_date} onChange={e=>setForm(p=>({...p,issued_date:e.target.value}))}/></div><div style={S.fg}><label style={S.label}>Due Date *</label><input style={S.input} type="date" value={form.due_date} onChange={e=>setForm(p=>({...p,due_date:e.target.value}))}/></div></div><div style={S.fg}><label style={S.label}>Priority</label><select style={S.select} value={form.priority} onChange={e=>setForm(p=>({...p,priority:e.target.value}))}>{["critical","high","medium","low"].map(p=><option key={p}>{p}</option>)}</select></div><div style={{display:"flex",gap:8,justifyContent:"flex-end"}}><button onClick={()=>setShowModal(false)} style={S.btnGhost}>Cancel</button><button onClick={save} disabled={saving} style={{...S.btn,opacity:saving?0.6:1}}>{saving?"Saving...":"Add"}</button></div></Modal>)}
  </div>);
}
...
function Reports({token}){
  ...
  const printReport=()=>{const w=window.open("","_blank");w.document.write(`<html><head><title>TaxPro Report</title><style>body{font-family:Arial;margin:20px;font-size:12px;}table{width:100%;border-collapse:collapse;}th,td{border:1px solid #ddd;padding:6px;}th{background:#1F6FEB;color:white;}</style></head><body><h2>TaxPro GST — ${rtype} Report</h2><p>Period: ${from} to ${to}</p>${document.getElementById("rpt")?.innerHTML||""}</body></html>`);w.document.close();w.print();};
  return(<div><div style={{display:"flex",gap:10,marginBottom:16,alignItems:"center",flexWrap:"wrap"}}><select style={{...S.select,width:"auto"}} value={rtype} onChange={e=>setRtype(e.target.value)}>{[{k:"gst-summary",l:"GST Summary"},{k:"sales-register",l:"Sales Register"},{k:"purchase-register",l:"Purchase Register"},{k:"outstanding",l:"Outstanding"},{k:"profit-loss",l:"Profit & Loss"},{k:"day-book",l:"Day Book"}].map(r=><option key={r.k} value={r.k}>{r.l}</option>)}</select><input type="date" style={{...S.input,width:150}} value={from} onChange={e=>setFrom(e.target.value)}/><span style={{color:"#8B949E"}}>to</span><input type="date" style={{...S.input,width:150}} value={to} onChange={e=>setTo(e.target.value)}/><button onClick={load} style={S.btn}>Generate</button>{data&&!data.error&&<button onClick={printReport} style={S.btnG}>🖨 Print</button>}</div>
    ...
    <div id="rpt">
      ...
      {data&&!data.error&&(rtype==="sales-register"||rtype==="purchase-register")&&data.invoices&&(<div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>{[{l:"Invoices",v:data.summary?.total_invoices},{l:"Taxable",v:fR(data.summary?.total_taxable)},{l:"Total Tax",v:fR((data.summary?.total_igst||0)+(data.summary?.total_cgst||0)+(data.summary?.total_sgst||0))},{l:"Total",v:fR(data.summary?.total_amount)}].map(k=>(<div key={k.l} style={{...S.kpi,textAlign:"center"}}><div style={S.kpiLabel}>{k.l}</div><div style={{fontSize:14,fontWeight:700,color:"#E6EDF3"}}>{k.v}</div></div>))}</div><div style={S.card}><table style={S.tbl}><thead><tr>{["Date","Invoice No","Party","GSTIN","Taxable","IGST","CGST","SGST","Total"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{data.invoices.map(inv=>(<tr key={inv.id}><td style={S.td}>{inv.invoice_date}</td><td style={{...S.td,color:"#58a6ff"}}>{inv.invoice_no}</td><td style={{...S.td,fontWeight:500,color:"#E6EDF3"}}>{inv.party_name}</td><td style={S.td}><span style={S.mono}>{inv.party_gstin||"—"}</span></td><td style={S.td}>{fR(inv.taxable_amount)}</td><td style={S.td}>{fR(inv.igst_amount)}</td><td style={S.td}>{fR(inv.cgst_amount)}</td><td style={S.td}>{fR(inv.sgst_amount)}</td><td style={{...S.tdL,fontWeight:700,color:"#3fb950"}}>{fR(inv.total_amount)}</td></tr>))}</tbody></table></div></div>)}
      {data&&!data.error&&rtype==="outstanding"&&(<div><div style={{...S.card,textAlign:"center",background:"#2d0e0e",border:"1px solid #6e1c1c",marginBottom:12}}><div style={S.kpiLabel}>Total Outstanding</div><div style={{fontSize:28,fontWeight:800,color:"#f85149"}}>{fR(data.total_outstanding)}</div></div><div style={S.card}><table style={S.tbl}><thead><tr>{["Party","GSTIN","Inv","Billed","Paid","Outstanding","Oldest Due"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(data.parties||[]).map((p,i)=>(<tr key={i}><td style={{...S.td,fontWeight:600,color:"#E6EDF3"}}>{p.party_name}</td><td style={S.td}><span style={S.mono}>{p.party_gstin||"—"}</span></td><td style={S.td}>{p.invoice_count}</td><td style={S.td}>{fR(p.total_billed)}</td><td style={{...S.td,color:"#3fb950"}}>{fR(p.total_paid)}</td><td style={{...S.td,color:"#f85149",fontWeight:700}}>{fR(p.outstanding)}</td><td style={S.tdL}>{p.oldest_due||"—"}</td></tr>))}</tbody></table></div></div>)}
...
function BankStatement({token,toast}){
  ...
  const TYPES=["INCOME","EXPENSE","PURCHASE","TAX","BANK","TRANSFER","UNKNOWN"];const TC={"INCOME":"green","EXPENSE":"red","PURCHASE":"blue","TAX":"amber","BANK":"gray","TRANSFER":"purple","UNKNOWN":"gray"};
...
function CompanyManager({token,toast,onSelect,selectedCompany}){
  ...
  useEffect(()=>{load();},[]);
  const openAdd=()=>{setEditing(null);setForm({name:"",legal_name:"",gstin:"",pan:"",address:"",city:"",state:"",pincode:"",phone:"",email:"",fy_start:"2024-04-01",fy_end:"2025-03-31",financial_year:"Apr-Mar"});setShowModal(true);};
  const openEdit=(e,c)=>{e.stopPropagation();setEditing(c);setForm({name:c.name,legal_name:c.legal_name||"",gstin:c.gstin||"",pan:c.pan||"",address:c.address||"",city:c.city||"",state:c.state||"",pincode:c.pincode||"",phone:c.phone||"",email:c.email||"",fy_start:c.fy_start||"2024-04-01",fy_end:c.fy_end||"2025-03-31",financial_year:c.financial_year||"Apr-Mar"});setShowModal(true);};
  ...
  return(<div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:12}}>
        {companies.map(c=>(<div key={c.id} onClick={()=>onSelect(c)} style={{...S.card,cursor:"pointer",border:`1px solid ${selectedCompany?.id===c.id?"#1F6FEB":"#21262D"}`,background:selectedCompany?.id===c.id?"#0c1d2e":"#161B22",transition:"all 0.2s"}}>
          <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:10}}>
            <div style={{fontSize:16,fontWeight:700,color:"#E6EDF3"}}>{c.name}</div>
          ...
          {c.gstin&&<div style={{fontSize:11,color:"#58a6ff",marginBottom:4}}><span style={S.mono}>{c.gstin}</span></div>}
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            ...
            <div style={{display:"flex",gap:6}}>
              <button onClick={e=>openEdit(e,c)} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Edit</button>
    ...
    {showModal&&(<Modal title={editing?"Edit Company":"Create Company"} onClose={()=>setShowModal(false)} wide>
      ...
      <div style={{...S.card,background:"#0c1d2e",border:"1px solid #1f4872",marginBottom:16}}>
        <div style={{fontSize:12,color:"#58a6ff"}}>✨ Creating a new company automatically sets up: Capital Account, Sundry Debtors/Creditors, Cash, Bank, Sales, Purchase, GST ledgers and all standard Chart of Accounts groups.</div>
      </div>
...
function LedgerGroups({token,toast,companyId}){
  ...
  const natureColor={Asset:"green",Liability:"red",Income:"teal",Expense:"amber"};
  return(<div>
    <div style={{display:"flex",gap:10,marginBottom:14,alignItems:"center"}}><div style={{fontSize:14,fontWeight:600,color:"#E6EDF3"}}>Chart of Accounts — Ledger Groups</div><button onClick={()=>{setEditing(null);setForm({name:"",parent_id:"",nature:"Asset",affects_gross:false});setShowModal(true);}} style={{...S.btn,marginLeft:"auto"}}>+ New Group</button></div>
    {loading?<Spinner/>:(<div style={S.twoCol}>{Object.entries(byNature).map(([nature,grps])=>(<div key={nature} style={S.card}><div style={{fontSize:13,fontWeight:700,marginBottom:10}}>{badge(nature,natureColor[nature])}</div>{grps.length===0?<div style={{color:"#8B949E",fontSize:12}}>No groups</div>:grps.map(g=>(<div key={g.id} style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #21262D"}}><div><div style={{fontSize:12,fontWeight:g.is_default?600:400,color:"#C9D1D9"}}>{g.name}</div>{g.parent_id&&<div style={{fontSize:10,color:"#8B949E"}}>Sub-group</div>}</div><div style={{display:"flex",gap:4,alignItems:"center"}}>{g.affects_gross&&badge("Gross","purple")}{!g.is_default&&<><button onClick={()=>{setEditing(g);setForm({name:g.name,parent_id:g.parent_id||"",nature:g.nature,affects_gross:g.affects_gross||false});setShowModal(true);}} style={{...S.btnGhost,fontSize:10,padding:"3px 6px"}}>Edit</button><button onClick={()=>del(g.id)} style={{...S.btnDanger,fontSize:10,padding:"3px 6px"}}>Del</button></>}</div></div>))}</div>))}</div>)}
    {showModal&&(<Modal title={editing?"Edit Group":"New Ledger Group"} onClose={()=>setShowModal(false)}>
...
function LedgerManager({token,toast,companyId}){
  ...
  const del=async id=>{if(!window.confirm("Delete ledger?"))return;try{await api(`/accounting/companies/${companyId}/ledgers/${id}`,"DELETE",null,token);toast("Deleted","success");load();}catch(e){toast(e.message,"error");}};
  ...
  return(<div>
    ...
    {loading?<Spinner/>:(<div style={S.card}>{ledgers.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No ledgers found.</div>:(<table style={S.tbl}><thead><tr>{["Ledger Name","Group","Nature","Opening","Balance","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{ledgers.map(l=>(<tr key={l.id}><td style={{...S.td,fontWeight:600,color:"#E6EDF3",cursor:"pointer"}} onClick={()=>viewStatement(l)}>{l.name}{l.is_default&&<span style={{fontSize:9,color:"#8B949E",marginLeft:6}}>DEFAULT</span>}</td><td style={S.td}>{l.group_name}</td><td style={S.td}>{badge(l.nature,natureColor[l.nature]||"gray")}</td><td style={S.td}>{fR(l.opening_balance)} <span style={{fontSize:10,color:"#8B949E"}}>{l.opening_type}</span></td><td style={{...S.td,fontWeight:700,color:l.balance_type==="Dr"?"#3fb950":"#f85149"}}>{fR(l.balance)} <span style={{fontSize:10}}>{l.balance_type}</span></td><td style={S.tdL}><div style={{display:"flex",gap:4}}><button onClick={()=>viewStatement(l)} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Statement</button>{!l.is_default&&<><button onClick={()=>{setEditing(l);setForm({name:l.name,group_id:l.group_id,opening_balance:l.opening_balance||"0",opening_type:l.opening_type||"Dr",alias:l.alias||"",gstin:l.gstin||"",pan:l.pan||"",address:l.address||"",phone:l.phone||"",email:l.email||"",bank_account:l.bank_account||"",bank_name:l.bank_name||"",ifsc_code:l.ifsc_code||"",credit_limit:l.credit_limit||"0",credit_days:l.credit_days||"0",notes:l.notes||""});setShowModal(true);}} style={{...S.btnGhost,fontSize:11,padding:"4px 8px"}}>Edit</button><button onClick={()=>del(l.id)} style={{...S.btnDanger,fontSize:11,padding:"4px 8px"}}>Del</button></>}</div></td></tr>))}</tbody></table>)}</div>)}
    {showModal&&(<Modal title={editing?"Edit Ledger":"Create Ledger"} onClose={()=>setShowModal(false)} wide>
      <div style={S.twoCol}>
        <div>
          <div style={S.fg}><label style={S.label}>Ledger Name *</label><input style={S.input} placeholder="e.g. HDFC Bank Current A/c" value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))}/></div>
          <div style={S.fg}><label style={S.label}>Group *</label><select style={S.select} value={form.group_id} onChange={e=>setForm(p=>({...p,group_id:e.target.value}))}><option value="">Select Group</option>{["Asset","Liability","Income","Expense"].map(n=>(<optgroup key={n} label={n}>{groups.filter(g=>g.nature===n).map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</optgroup>))}</select></div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
          ...
          <div style={S.fg}><label style={S.label}>Alias</label><input style={S.input} value={form.alias} onChange={e=>setForm(p=>({...p,alias:e.target.value}))}/></div>
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
            <div style={S.fg}><label style={S.label}>Credit Limit</label><input style={S.input} type="number" value={form.credit_limit} onChange={e=>setForm(p=>({...p,credit_limit:e.target.value}))}/></div>
            <div style={S.fg}><label style={S.label}>Credit Days</label><input style={S.input} type="number" value={form.credit_days} onChange={e=>setForm(p=>({...p,credit_days:e.target.value}))}/></div>
          </div>
        </div>
      </div>
    ...
    {statement&&(<Modal title={`Ledger Statement — ${statement.ledger_name}`} onClose={()=>setStatement(null)} wide>
      ...
      <tbody>
        ...
        {(statement.transactions||[]).map((t,i)=>(<tr key={i}><td style={S.td}>{t.date}</td><td style={{...S.td,color:"#58a6ff"}}>{t.voucher_no}</td><td style={S.td}>{badge(t.voucher_type,"gray")}</td><td style={{...S.td,maxWidth:180}}><div style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.narration||t.v_narration||"—"}</div></td><td style={{...S.td,color:"#3fb950"}}>{t.dr_amount>0?fR(t.dr_amount):"—"}</td><td style={{...S.td,color:"#f85149"}}>{t.cr_amount>0?fR(t.cr_amount):"—"}</td><td style={{...S.tdL,fontWeight:600,color:t.balance_type==="Dr"?"#3fb950":"#f85149"}}>{fR(t.running_balance)} {t.balance_type}</td></tr>))}
        {!statement.transactions?.length&&<tr><td colSpan={7} style={{...S.td,textAlign:"center",color:"#8B949E",padding:20}}>No transactions</td></tr>}
      </tbody></table>
    </Modal>)}
  </div>);
}
...
function AccountingReports({token,toast,companyId}){
  ...
  const load=async()=>{
    ...
    try{
      let url=`/accounting/companies/${companyId}/reports/${rtype}`;
      if(rtype==="balance-sheet")url+=`?as_on_date=${asOnDate}`;
      else if(rtype==="day-book")url+=`?date=${asOnDate}`;
      else url+=`?from_date=${fromDate}&to_date=${toDate}`;
      const d=await api(url,"GET",null,token);setData(d);
    }catch(e){toast(e.message,"error");}
    setLoading(false);
  };
  const printR=()=>{const w=window.open("","_blank");w.document.write(`<html><head><title>TaxPro — ${rtype}</title><style>body{font-family:Arial;margin:20px;font-size:12px;}h2{color:#1F6FEB;}table{width:100%;border-collapse:collapse;margin:10px 0;}th,td{border:1px solid #ddd;padding:7px;}th{background:#1F6FEB;color:white;}.dr{color:green;}.cr{color:red;}.total{font-weight:bold;background:#f0f0f0;}</style></head><body><h2>TaxPro Accounting — ${rtype.replace(/-/g," ").toUpperCase()}</h2><p>Period: ${fromDate} to ${toDate}</p>${document.getElementById("acc-rpt")?.innerHTML||""}</body></html>`);w.document.close();w.print();};
  ...
  return(<div>
    ...
    <div id="acc-rpt">
      {/* TRIAL BALANCE */}
      {data&&rtype==="trial-balance"&&(<div><div style={{display:"flex",gap:10,marginBottom:12,alignItems:"center"}}>{badge(data.totals?.balanced?"Balanced ✓":"Not Balanced ⚠",data.totals?.balanced?"green":"red")}<span style={{color:"#8B949E",fontSize:12}}>Dr Total: <span style={{color:"#3fb950",fontWeight:700}}>{fR(data.totals?.dr)}</span> &nbsp; Cr Total: <span style={{color:"#f85149",fontWeight:700}}>{fR(data.totals?.cr)}</span></span></div><div style={S.card}><table style={S.tbl}><thead><tr>{["Ledger","Group","Nature","Dr Amount","Cr Amount"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(data.rows||[]).map((r,i)=>(<tr key={i}><td style={{...S.td,fontWeight:600,color:"#E6EDF3"}}>{r.name}</td><td style={S.td}>{r.group}</td><td style={S.td}>{badge(r.nature,{Asset:"green",Liability:"red",Income:"teal",Expense:"amber"}[r.nature]||"gray")}</td><td style={{...S.td,color:"#3fb950",fontWeight:r.dr_amount>0?700:400}}>{r.dr_amount>0?fR(r.dr_amount):"—"}</td><td style={{...S.tdL,color:"#f85149",fontWeight:r.cr_amount>0?700:400}}>{r.cr_amount>0?fR(r.cr_amount):"—"}</td></tr>))}</tbody><tfoot><tr className="total"><td colSpan={3} style={{...S.td,fontWeight:700}}>TOTAL</td><td style={{...S.td,color:"#3fb950",fontWeight:700}}>{fR(data.totals?.dr)}</td><td style={{...S.tdL,color:"#f85149",fontWeight:700}}>{fR(data.totals?.cr)}</td></tr></tfoot></table></div></div>)}
      {/* PROFIT & LOSS */}
      {data&&rtype==="profit-loss"&&data.report&&(<div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
        ...
        <div style={{...S.card,background:data.report.net_profit>=0?"#0d2818":"#2d0e0e",border:`1px solid ${data.report.net_profit>=0?"#238636":"#6e1c1c"}`,textAlign:"center"}}>
          <div style={{display:"flex",justifyContent:"space-around"}}><div><div style={S.kpiLabel}>Gross Profit</div><div style={{fontSize:20,fontWeight:800,color:data.report.gross_profit>=0?"#3fb950":"#f85149"}}>{fR(data.report.gross_profit)}</div></div><div style={{borderLeft:"1px solid #30363D"}}></div><div><div style={S.kpiLabel}>Net Profit / Loss</div><div style={{fontSize:26,fontWeight:800,color:data.report.net_profit>=0?"#3fb950":"#f85149"}}>{fR(data.report.net_profit)}</div></div></div>
        </div>
      </div>)}
      {/* BALANCE SHEET */}
      {data&&rtype==="balance-sheet"&&(<div><div style={{display:"flex",gap:10,marginBottom:12,alignItems:"center"}}>{badge(`As on: ${data.as_on_date}`,"blue")}{badge(Math.abs(data.difference||0)<0.01?"Balanced ✓":"Not Balanced",Math.abs(data.difference||0)<0.01?"green":"red")}</div><div style={S.twoCol}>
        <div style={S.card}><div style={{fontSize:13,fontWeight:700,color:"#58a6ff",marginBottom:10}}>ASSETS</div>{(data.assets||[]).map((a,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #21262D"}}><div><div style={{fontSize:12,fontWeight:500,color:"#E6EDF3"}}>{a.name}</div><div style={{fontSize:10,color:"#8B949E"}}>{a.group}</div></div><span style={{color:"#58a6ff",fontWeight:600}}>{fR(a.balance)} <span style={{fontSize:10}}>{a.balance_type}</span></span></div>))}<div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",fontWeight:700,borderTop:"2px solid #30363D",marginTop:4}}><span>Total Assets</span><span style={{color:"#58a6ff"}}>{fR(data.total_assets)}</span></div></div>
        <div style={S.card}><div style={{fontSize:13,fontWeight:700,color:"#f85149",marginBottom:10}}>LIABILITIES</div>{(data.liabilities||[]).map((l,i)=>(<div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid #21262D"}}><div><div style={{fontSize:12,fontWeight:500,color:"#E6EDF3"}}>{l.name}</div><div style={{fontSize:10,color:"#8B949E"}}>{l.group}</div></div><span style={{color:"#f85149",fontWeight:600}}>{fR(l.balance)} <span style={{fontSize:10}}>{l.balance_type}</span></span></div>))}<div style={{display:"flex",justifyContent:"space-between",padding:"8px 0",fontWeight:700,borderTop:"2px solid #30363D",marginTop:4}}><span>Total Liabilities</span><span style={{color:"#f85149"}}>{fR(data.total_liabilities)}</span></div></div>
      </div></div>)}
      {/* DAY BOOK */}
      {data&&rtype==="day-book"&&(<div><div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:12}}>{[{l:"Vouchers",v:data.summary?.total_vouchers,c:"#58a6ff"},{l:"Total Dr",v:fR(data.summary?.total_dr),c:"#3fb950"},{l:"Total Cr",v:fR(data.summary?.total_cr),c:"#f85149"}].map(k=>(<div key={k.l} style={{...S.kpi,textAlign:"center"}}><div style={S.kpiLabel}>{k.l}</div><div style={{fontSize:k.l==="Vouchers"?22:14,fontWeight:700,color:k.c}}>{k.v}</div></div>))}</div><div style={S.card}>{data.vouchers?.length===0?<div style={{textAlign:"center",padding:30,color:"#8B949E"}}>No vouchers on {asOnDate}</div>:data.vouchers?.map(v=>(<div key={v.id} style={{marginBottom:16,paddingBottom:12,borderBottom:"1px solid #21262D"}}><div style={{display:"flex",gap:8,marginBottom:8,alignItems:"center"}}><span style={{color:"#58a6ff",fontWeight:700}}>{v.voucher_no}</span>{badge(v.voucher_type,"gray")}{v.party_name&&<span style={{color:"#8B949E",fontSize:11}}>{v.party_name}</span>}{v.narration&&<span style={{color:"#8B949E",fontSize:11,marginLeft:"auto"}}>{v.narration}</span>}</div><table style={S.tbl}><thead><tr>{["Ledger","Dr","Cr"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(v.items||[]).map((item,i)=>(<tr key={i}><td style={S.td}>{item.ledger_name}</td><td style={{...S.td,color:"#3fb950"}}>{item.dr_amount>0?fR(item.dr_amount):"—"}</td><td style={{...S.tdL,color:"#f85149"}}>{item.cr_amount>0?fR(item.cr_amount):"—"}</td></tr>))}</tbody></table></div>))}</div></div>)}
      {/* CASH BOOK */}
      {data&&rtype==="cash-book"&&(<div><div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:12}}>{[{l:"Opening",v:fR(data.summary?.opening),c:"#8B949E"},{l:"Total Receipts",v:fR(data.summary?.total_receipts),c:"#3fb950"},{l:"Total Payments",v:fR(data.summary?.total_payments),c:"#f85149"},{l:"Closing",v:`${fR(data.summary?.closing)} ${data.summary?.closing_type}`,c:"#e3b341"}].map(k=>(<div key={k.l} style={{...S.kpi,textAlign:"center"}}><div style={S.kpiLabel}>{k.l}</div><div style={{fontSize:k.l==="Opening"||k.l==="Closing"?12:13,fontWeight:700,color:k.c}}>{k.v}</div></div>))}</div><div style={S.card}><table style={S.tbl}><thead><tr>{["Date","Voucher No","Type","Narration","Receipt (Dr)","Payment (Cr)"].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{(data.transactions||[]).map((t,i)=>(<tr key={i}><td style={S.td}>{t.date}</td><td style={{...S.td,color:"#58a6ff"}}>{t.voucher_no}</td><td style={S.td}>{badge(t.voucher_type,"gray")}</td><td style={{...S.td,color:"#8B949E"}}>{t.v_narration||t.party_name||"—"}</td><td style={{...S.td,color:"#3fb950",fontWeight:t.dr_amount>0?700:400}}>{t.dr_amount>0?fR(t.dr_amount):"—"}</td><td style={{...S.tdL,color:"#f85149",fontWeight:t.cr_amount>0?700:400}}>{t.cr_amount>0?fR(t.cr_amount):"—"}</td></tr>))}{!data.transactions?.length&&<tr><td colSpan={6} style={{...S.td,textAlign:"center",color:"#8B949E",padding:20}}>No transactions</td></tr>}</tbody></table></div></div>)}
    </div>
  </div>);
}
...
function AccountingShell({token,toast,activeView}){
  ...
  if(!company)return(<div style={{...S.card,textAlign:"center",padding:50}}><div style={{fontSize:48,marginBottom:12}}>🏢</div><div style={{fontSize:16,fontWeight:700,color:"#E6EDF3",marginBottom:8}}>No Company Selected</div><div style={{color:"#8B949E",fontSize:13,marginBottom:20}}>Go to "Companies" and create or select a company to use accounting features.</div><button onClick={()=>{}} style={S.btn}>← Go to Companies</button></div>);
  return(<div>
    <div style={{...S.card,background:"#0c1d2e",border:"1px solid #1f4872",marginBottom:14,padding:"10px 14px",display:"flex",alignItems:"center",gap:10}}>
      ...
      {company.gstin&&<span style={{...S.mono,fontSize:11}}>{company.gstin}</span>}
...
function Reconciliation({token,toast}){
  ...
  return(<div><div style={{display:"flex",gap:12,marginBottom:14,alignItems:"center",flexWrap:"wrap"}}><select style={{...S.select,width:"auto"}} value={clientId} onChange={e=>setClientId(e.target.value)}>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select><select style={{...S.select,width:"auto"}} value={period} onChange={e=>setPeriod(e.target.value)}>{["FY 2024-25","FY 2023-24","FY 2022-23"].map(p=><option key={p}>{p}</option>)}</select><button onClick={()=>setShowModal(true)} style={{...S.btn,marginLeft:"auto"}}>+ Add Entry</button></div>
    ...
    {loading?<Spinner/>:(<div style={S.card}>{!data||data.rows.length===0?<div style={{textAlign:"center",padding:40,color:"#8B949E"}}>No entries.</div>:(<table style={S.tbl}><thead><tr>{["Vendor","GSTIN","Inv","GSTR-2A","GSTR-2B","Books","Diff","Status",""].map(h=><th key={h} style={S.th}>{h}</th>)}</tr></thead><tbody>{data.rows.map(r=>(<tr key={r.id}><td style={{...S.td,fontWeight:500,color:"#E6EDF3"}}>{r.vendor_name}</td><td style={S.td}><span style={S.mono}>{r.vendor_gstin}</span></td><td style={S.td}>{r.invoice_count}</td><td style={S.td}>{fR(r.gstr2a_amount)}</td><td style={S.td}>{fR(r.gstr2b_amount)}</td><td style={S.td}>{fR(r.books_amount)}</td><td style={{...S.td,fontWeight:r.difference!==0?700:400,color:r.difference<0?"#f85149":r.difference>0?"#e3b341":"#3fb950"}}>{r.difference===0?"—":fR(r.difference)}</td><td style={S.td}>{r.status==="matched"?badge("Matched","green"):r.status==="mismatch"?badge("Mismatch","amber"):badge("Missing","red")}</td><td style={S.tdL}><button onClick={()=>del(r.id)} style={S.btnDanger}>Del</button></td></tr>))}</tbody></table>)}</div>)}
...
function GSTR2AImport({token,toast}){
  ...
  return(<div><div style={{display:"flex",gap:0,marginBottom:20}}>{[{n:1,l:"Upload"},{n:2,l:"Preview"},{n:3,l:"Done"}].map((s,i)=>(<div key={s.n} style={{display:"flex",alignItems:"center",flex:1}}><div style={{display:"flex",flexDirection:"column",alignItems:"center",flex:1}}><div style={{width:30,height:30,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,fontSize:12,background:step>=s.n?"#1F6FEB":"#21262D",color:step>=s.n?"#fff":"#8B949E"}}>{step>s.n?"✓":s.n}</div><div style={{fontSize:11,color:step>=s.n?"#58a6ff":"#8B949E",marginTop:4}}>{s.l}</div></div>{i<2&&<div style={{height:2,flex:1,background:step>s.n?"#1F6FEB":"#21262D",marginBottom:18}}/>}</div>))}</div>
    {step===1&&(<div><div style={{...S.card,background:"#0c1d2e",border:"1px solid #1f4872",marginBottom:14}}><div style={{fontSize:13,fontWeight:600,color:"#58a6ff",marginBottom:8}}>How to download GSTR-2A</div>{["Login to gst.gov.in","Services → Returns → Returns Dashboard","Select FY and Period","GSTR-2A → Download/View","Download Excel and upload below"].map((s,i)=>(<div key={i} style={{display:"flex",gap:10,padding:"3px 0",fontSize:12,color:"#C9D1D9"}}><span style={{color:"#1F6FEB",fontWeight:700}}>{i+1}.</span><span>{s}</span></div>))}</div><div style={S.card}><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:14}}><div><label style={S.label}>Client *</label><select style={S.select} value={clientId} onChange={e=>setClientId(e.target.value)}><option value="">Select</option>{clients.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></div><div><label style={S.label}>Period *</label><select style={S.select} value={period} onChange={e=>setPeriod(e.target.value)}>{["FY 2024-25","FY 2023-24","FY 2022-23"].map(p=><option key={p}>{p}</option>)}</select></div></div><div style={{border:"2px dashed #30363D",borderRadius:10,padding:30,textAlign:"center",marginBottom:16,background:file?"#0d2818":"#0D1117",borderColor:file?"#238636":"#30363D"}}>{file?(<div><div style={{fontSize:28,marginBottom:8}}>✅</div><div style={{fontSize:13,fontWeight:600,color:"#3fb950"}}>{file.name}</div><button onClick={()=>setFile(null)} style={{...S.btnGhost,marginTop:10,fontSize:11}}>Remove</button></div>):(<div><div style={{fontSize:36,marginBottom:8}}>📁</div><div style={{fontSize:13,color:"#C9D1D9",marginBottom:12}}>GSTR-2A Excel / JSON file</div><label style={{...S.btn,cursor:"pointer",display:"inline-block"}}>Choose File<input type="file" accept=".xlsx,.xls,.json,.csv" onChange={e=>setFile(e.target.files[0])} style={{display:"none"}}/></label></div>)}</div><button onClick={previewFile} disabled={!file||loading} style={{...S.btn,width:"100%",opacity:!file||loading?0.5:1}}>{loading?"Reading...":"Preview →"}</button></div></div>)}
...
export default function App(){
  ...
  const ACCOUNTING_VIEWS=["acc-companies","acc-groups","acc-ledgers","acc-vouchers","acc-reports"];
  const isAccounting=ACCOUNTING_VIEWS.includes(view);
  ...
  return(<div style={S.app}>
    ...
    <aside style={{...S.sidebar,width:collapsed?58:220,minWidth:collapsed?58:220,transition:"width 0.2s"}}>
      ...
      <nav style={{flex:1,padding:"4px 0",overflowY:"auto"}}>
        {["MAIN","ACCOUNTING","GST","TALLY","TOOLS"].map(g=>(
          <div key={g}>
            {!collapsed&&<div style={{fontSize:9,color:"#444C56",padding:"8px 12px 2px",letterSpacing:1,fontWeight:600}}>{g==="TALLY"?"TALLY ACCOUNTING":g}</div>}
    ...
    <div style={S.main}>
      ...
      <div style={S.content}>
        ...
        {isAccounting          &&<AccountingShell    token={token} toast={showToast} activeView={view}/>}
